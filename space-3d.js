...
function list(checked, li) {
    ...
    for (let i = 0; i < check.length; i++) {
        ...
        setTimeout(() => {
            if (checked == check[i]) {
                sun.material.map = new THREE.TextureLoader().load(Apperance[i]);
            }
        }, 100);
    }
...
function I_active() {
    ...
    let I_S_info;
    let I_width;
    if (screen_width <= 450 && screen_width > 0) {
        I_logo = '50%';
        I_S_info = '60%';
        I_width = '170px';
    }
    if (screen_width <= 600 && screen_width > 450) {
        I_logo = '55%';
        I_S_info = '65%';
        I_width = '190px';
    }
    if (screen_width <= 700 && screen_width > 600) {
        I_logo = '62%';
        I_S_info = '69%';
        I_width = '190px';
    }
    if (screen_width <= 800 && screen_width > 700) {
        I_logo = '69%';
        I_S_info = '75%';
        I_width = '190px';
    }
    if (screen_width <= 1000 && screen_width > 800) {
        I_logo = '67%';
        I_S_info = '73%';
        I_width = '220px';
    }
    ...
    if (screen_width <= screen_width && screen_width > 1200) {
        I_logo = '77%';
        I_S_info = '80.5%';
        I_width = '250px';
    }
...
function closeer() {
    ...
    if (screen_width <= screen_width && screen_width > 1100) {
        I_logo = '96%';
    }
...
function Choose_page(content, page) {
    ...
    for (let L = 1; L < 5; L++) {
        ...
        if (Margin_page == H_pages[L]) {
            footer.style.marginTop = Top_O[L]
        }
    }

    if (Margin_page == 'faqs') {
        footer.style.marginTop = F_fqs;
    }
    if (Margin_page == 'home') {
        footer.style.marginTop = home_mode[0];
    }
}
...
function information_choose(bar_info) {
    ...
    if (O_P == 'open') {
        ...
        for (let i = 0; i < Top_O.length; i++) {
            if (Margin_page == H_pages[i]) {
                footer.style.marginTop = Top_O[i]
            }
        }
        O_P = 'close';
        footer_mode = 'open';
        ...
        for (let i = 0; i < Top_C.length; i++) {
            if (Margin_page == H_pages[i]) {
                footer.style.marginTop = Top_C[i]
            }
        }
        O_P = 'open';
        footer_mode = 'close';
...
function Show_info_faqs(info, showinfo) {
    ...
    if (mode_show.includes(info) == false) {
        ...
        if (mode_inner == 'delet') {
            for (let o = show.length - 1; o > -1; o--) {
                setTimeout(() => {
                    show.splice(o, 1);
                    clicked.innerHTML = '';
                    for (let h = 0; h < show.length; h++) {
                        clicked.innerHTML += show[h];
                    }
                }, 50 * loop);
                loop++;
            }
        }
        if (loop == 15 && mode_inner == 'delet') {
            loop = 0;
            mode_inner = 'write'
        };
        setTimeout(() => {
        ...
        for (let b = 0; b < mode_show.length; b++) {
            if (mode_show[b] != '' && mode_show[b] != undefined) {
                F_margin_T.push(mode_show[b]);
            }
        }
    } else {
    ...
    for (let T = 0; T < F_margin_T.length; T++) {
        if (F_margin_T.length > 0) {
            if (F_margin_T.length == (T + 1)) {
                footer.style.marginTop = F_T_M[T];
                F_fqs = F_T_M[T];
            }
        }
    }
    if (F_margin_T.length == 0 && footer_mode == 'close') {
        footer.style.marginTop = '900px';
        F_fqs = '900px';
    }
    if (F_margin_T.length == 0 && footer_mode == 'open') {
